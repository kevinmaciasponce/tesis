...
export class EditBeneficiaryComponent implements OnInit {
 ...
 onSubmit2() {
  ...
  guardaValoracion(){
    ...
    this.sponsorService.editaValoracion(JSON.stringify(this.obtenerDatos()) ,this.filefrontal3!,this.user).subscribe( 
      (data:any)=>{
        console.log(data)
    ...
    if(this.existeValoracion()){
      Swal.fire({
        ...
        ()=>{
          ...
          setTimeout(() => {
            this.cargando=false;
            this.router.navigate(["/beneficiary/show"]);
          }, 1000);
        }
      )
    }else{
  ...
  async convertToFile(src:any,fileName:any,mimeType:any){
  ...
  getConsulta(){
    return {
        "id": '',
        "nomApe":'',
        "identificacion":this.datosdeportista.get('cedula')?.value as string,
        "disciplina":'',
        "estado":"PC"
      }
  }
  ...
  consultarBeneficiario(){
    ...
    this.sponsorService.consultaAuspiciosPorConfirmar(this.getConsulta()).subscribe(
      (data:any)=>{
        console.log(data);
        this.datosDeportista=data[0];
        if(this.datosDeportista.persona){
        ...
        if(this.datosDeportista.beneficiario){
          this.urlphoto=this.datosDeportista.beneficiario.ruta1;
          this.urlphoto2= this.datosDeportista.beneficiario.ruta2;
          ...
          setTimeout(() => {
            console.log(this.data(this.sanitizer.bypassSecurityTrustUrl(this.urlphoto)));
            console.log(this.datosDeportista.beneficiario.banco);
            ...
            console.log(this.datosDeportista.beneficiario.tipoCuenta);
  ...
  data(url2:any){
    const toDataURL = (url:any) => fetch(url2)
     ...
     .then(dataUrl => {
        ...
        var fileData = this.dataURLtoFile(dataUrl, "imageName.jpg");
  ...
  getDatosNatural(){
    return {
      ...
      idRepresentante:   this.datosDeportista.beneficiario.representante,
  ...
  guardarDatos(cuenta:any){
    ...
    this.cargando=true;
    this.sponsorService.EditarBeneficiario(cuenta,this.user).subscribe(
      (data:any)=>{
       console.log(data);
      }, (error:Error)=>{
        this.cargando=false;
...
console.log(this.comprobarNombreCompetencia());
  ...
  enviardatos(){
    ...
    let cuenta= {
        identificacion: this.db['cedula'].value as string,
        idRepre:   this.datosDeportista.beneficiario.representante,
        titulos:titulos
    }
    this.cargando=true;
    this.sponsorService.EditarTitulos(cuenta,this.user).subscribe(
      (datos:any)=>{
        console.log(datos);
      },(error:Error)=>{
        this.cargando=false;
