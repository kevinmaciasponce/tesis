...
editarAuspicio(){
  ...
  this.sponsorService.EditarAuspicio(this.ObtenerAuspicio(),this.user).subscribe(
    (data:any)=>{
      console.log(data);
    },(error:Error)=>{
      this.cargando=false;
...
agregarRecompensas(){
  ...
  this.sponsorService.EditarRecompensas(this.getRecompensas(),this.user).subscribe(
    (data:any)=>{
      console.log(data);
    },(error:Error)=>{
      console.log(error);
...
consultaBeneficiarioAuspicio(){
  this.cargando=true;
  this.sponsorService.consultaAuspiciosPorConfirmar(this.obtenerDatosAuspicio()).subscribe(
    (data:any)=>{
      console.log(data);
      ...
      this.dbauspicio['presupuesto'].setValue(data[0].auspicio.montoSolicitado);
    },(error:Error)=>{
      this.cargando=false;
      console.log(error);
    },()=>{
...
agregarTorneos(){
  this.cargando=true;
  this.sponsorService.EditarTorneos(this.getTorneos(),this.user).subscribe(
    (data:any)=>{
      console.log(data);
    },(error:Error)=>{
      this.cargando=false;
...
getRecompensas(){
  return  {
    'numeroAuspicio':this.nAuspicio as string,
