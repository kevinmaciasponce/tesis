...
export class InvestorService {
  ...
  GuardarIdentificaion(datos: any,files:File,filepost:File,user: User): Observable<any>{
   ...
   formData.append('filepost',filepost);
    ...
    console.log(datos);
    this.API_SERVER = environment.URL_BASE + 'multiplo/investor/formularios/documentos/cedula';
  ...
  GuardarComprobante(datos: any,files:File,user: User): Observable<any>{
   ...
   formData.append('file',files);
    ...
    console.log(datos);
   // this.API_SERVER = environment.URL_BASE + 'multiplo/loquesea2';
    this.API_SERVER = environment.URL_BASE + 'multiplo/investor/formularios/documentos/deposito';
    return this.http.post<any>(this.API_SERVER,formData,httpOptions).pipe(
      catchError(this.handleError)
    );
  }
  ...
  //Cambiar a formdata
  consultaDatosInversionista(cuenta:any): Observable<any> {
    this.API_SERVER = environment.URL_BASE + 'multiplo/consultas/persona';
    const headers: any = {
      "Authorization": 'Bearer ' +cuenta.token
   };
   const formData = new FormData();
   formData.append('identificacion', cuenta.identificacion);
   //Post options pass it to HttpHeaders Class 
    const httpOptions = {
       headers: new HttpHeaders(headers),
   };
  ...
  consultaCedulaInversionista(cuenta:any): Observable<any> {
    this.API_SERVER = environment.URL_BASE + 'multiplo/consultas/persona/documento/identificacion';
    const headers: any = {
      "Authorization": 'Bearer ' +cuenta.token
   };
   const formData = new FormData();
   formData.append('identificacion', cuenta.user.identificacion);
   //Post options pass it to HttpHeaders Class 
    const httpOptions = {
       headers: new HttpHeaders(headers),
   };
