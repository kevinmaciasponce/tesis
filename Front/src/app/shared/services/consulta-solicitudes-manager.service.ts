...
export class ConsultaSolicitudesManagerService {
  ...
  ManagerIntransit(form: any,user:User): Observable<any> {
    this.API_SERVER = environment.URL_BASE + 'multiplo/solicitud/inversiones/consultas/int/gerencia/enTransito';
    const headers: any = {
  ...
  ManagerIntransitDetails(form: any,user:User): Observable<any> {
    this.API_SERVER = environment.URL_BASE + 'multiplo/solicitud/inversiones/consultas/int/solProyecto';
    const headers: any = {
      "Authorization": 'Bearer ' +user.token
   };
